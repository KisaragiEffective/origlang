    ...
    fn type_check(self, checker: &TypeChecker) -> Result<Self::Success, Self::Err> {
        match self {
            ...
            Self::BinaryOperator { lhs, rhs, operator } => {
                ...
                match operator {
                    BinaryOperatorKind::Plus => match (lhs_type, rhs_type) {
                        (Type::GenericInteger, Type::GenericInteger) => {
                        ...
                        (other_lhs, other_rhs) => {
                            Err(TypeChecker::invalid_combination_for_binary_operator(
                                Type::GenericInteger,
                                operator,
                                Type::GenericInteger,
                                other_lhs,
    ...
    match element_binding {
        ...
        SinglePattern::Bind(identifier) => {
            checker
                .ctx
                .borrow_mut()
                .add_known_variable(identifier.clone(), expr.actual_type());
            ...
            TypedExpression::Variable { ident, tp } => {
                ...
                if outer_destruction.len() == tuple_element_types.len() {
                    rhs = TypedExpression::Tuple {
                        expressions: tuple_element_types
                            .iter()
                            .enumerate()
                            .map(|(i, _)| TypedExpression::ExtractTuple {
            ...
            TypedExpression::ExtractTuple { expr, index } => {
                ...
                let expr = match expr {
                    TypedExpression::If {
                        ...
                        els,
                        return_type,
                    } => RecursivePatternMatchDesugarStrategy::Simple(TypedExpression::If {
                        condition,
                        then,
                        els,
                        return_type: return_type.as_tuple().expect("oops 15").0[index].clone(),
                    }),
                    TypedExpression::Block {
                        inner,
                        final_expression,
                        return_type,
                    } => RecursivePatternMatchDesugarStrategy::Simple(TypedExpression::Block {
                        inner,
                        final_expression,
                        return_type: return_type.as_tuple().expect("oops 4").0[index].clone(),
                    }),
                    TypedExpression::Tuple { expressions } => {
                ...
                match expr {
                    ...
                    RecursivePatternMatchDesugarStrategy::InsertTemporary(expr) => {
                        ...
                        checker
                            .ctx
                            .borrow_mut()
                            .add_known_variable(new_ident.clone(), tp);
...
fn extract_pattern(
    ...
    let Ok(dest) = checker.lower_type_signature_into_type(&type_name) else {
        return Err(TypeCheckError::UnknownType { name: type_name });
    };

    match dest.is_assignable(&expr.actual_type()) {
        AssignableQueryAnswer::Yes => handle_atomic_pattern(expr, pattern, checker),
        ...
        AssignableQueryAnswer::No => Err(TypeCheckError::UnassignableType {
            from: expr.actual_type(),
            to: dest,
        }),
    }
...
impl TryIntoTypeCheckedForm for Statement {
    ...
    fn type_check(self, checker: &TypeChecker) -> Result<Self::Success, Self::Err> {
        match self {
            Self::Print { expression } => checker
                .check(expression)
                .map(|e| vec![TypedStatement::Print { expression: e }]),
            ...
            Self::TypeAliasDeclaration {
                ...
                checker.ctx.borrow_mut().known_aliases.insert(
                    new_name,
                    checker
                        .lower_type_signature_into_type(&replace_with)
                        .map_err(|()| TypeCheckError::UnknownType { name: replace_with })?,
                );
...
impl TryIntoTypeCheckedForm for RootAst {
    ...
    fn type_check(self, checker: &TypeChecker) -> Result<Self::Success, Self::Err> {
        ...
        Ok(TypedRootAst { statements: vec })
    }
}
...
impl TypeChecker {
    ...
    pub fn lower_type_signature_into_type(&self, p0: &TypeSignature) -> Result<Type, ()> {
        match p0 {
            TypeSignature::Simple(ident) => match ident.as_name() {
...
impl TypeChecker {
    const fn invalid_combination_for_binary_operator(
        accepted_lhs: Type,
...
impl Context {
    ...
    fn lookup_variable_type(&self, variable_name: &Identifier) -> Result<Type, TypeCheckError> {
        self.known_typed_variables
            .get(variable_name)
            .cloned()
            .ok_or_else(|| TypeCheckError::UndefinedIdentifier(variable_name.clone()))
    }
