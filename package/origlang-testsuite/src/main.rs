...
impl Test {
    ...
    fn evaluated_expressions_with_optimization_preset(
        src: &str,
        preset: &dyn EachStep,
    ) -> Result<Vec<TypeBox>, Err> {
    ...
    fn print_literal() {
        assert_eq!(
            Self::evaluated_expressions("print 123456\n").expect("properly parsed and typed"),
            type_boxes![123456 => NonCoercedInteger]
        );
        assert_eq!(
            Self::evaluated_expressions("print 1\nprint 2\n").expect("properly parsed and typed"),
    ...
    fn simple_variable_assignment() {
        assert_eq!(
            Self::evaluated_expressions("var x = 1\nprint x\n").expect("properly parsed and typed"),
    ...
    fn op_plus() {
        assert_eq!(
            Self::evaluated_expressions("print 1 + 2\n").expect("properly parsed and typed"),
    ...
    fn expr_parenthesised() {
        ...
        assert_eq!(
            Self::evaluated_expressions("print 3 - (2 - 1)\n").expect("properly parsed and typed"),
            type_boxes![2 => NonCoercedInteger]
        );
        assert_eq!(
            Self::evaluated_expressions("print (3 - 2) - 1\n").expect("properly parsed and typed"),
    ...
    fn op_multiply() {
        // multiply test
        assert_eq!(
            Self::evaluated_expressions("print 3 * 2\n").expect("properly parsed and typed"),
            type_boxes![6 => NonCoercedInteger]
        );
        assert_eq!(
            Self::evaluated_expressions("print 3 * 2 + 1\n").expect("properly parsed and typed"),
    ...
    fn literal_bool() {
        ...
        assert_eq!(
            Self::evaluated_expressions("print true\n").expect("properly parsed and typed"),
        ...
        assert_eq!(
            Self::evaluated_expressions("print false\n").expect("properly parsed and typed"),
            type_boxes![false => Boolean]
        );
        assert_ne!(
            Self::evaluated_expressions("print false\n").expect("properly parsed and typed"),
    ...
    fn test_less() {
        // less equal
        assert_eq!(
            Self::evaluated_expressions("print 1 <= 0\n").expect("properly parsed and typed"),
            type_boxes![false => Boolean]
        );
        assert_eq!(
            Self::evaluated_expressions("print 1 <= 1\n").expect("properly parsed and typed"),
            type_boxes![true => Boolean]
        );
        assert_eq!(
            Self::evaluated_expressions("print 1 <= 2\n").expect("properly parsed and typed"),
        ...
        assert_eq!(
            Self::evaluated_expressions("print 1 <= 0 == 0 >= 1\n")
                .expect("properly parsed and typed"),
            type_boxes![true => Boolean]
        );
        assert_eq!(
            Self::evaluated_expressions("print 1 <= 1 == 1 >= 1\n")
                .expect("properly parsed and typed"),
            type_boxes![true => Boolean]
        );
        assert_eq!(
            Self::evaluated_expressions("print 1 <= 2 == 2 >= 1\n")
                .expect("properly parsed and typed"),
            type_boxes![true => Boolean]
        );
    ...
    fn test_equality_operator() {
        assert_eq!(
            Self::evaluated_expressions("print 42 == 42\n").expect("properly parsed and typed"),
            type_boxes![true => Boolean]
        );
        assert_eq!(
            Self::evaluated_expressions("print 42 == 21\n").expect("properly parsed and typed"),
            type_boxes![false => Boolean]
        );
        assert_eq!(
            Self::evaluated_expressions("print 42 != 42\n").expect("properly parsed and typed"),
    ...
    fn test_if_expression() {
        assert_eq!(
            Self::evaluated_expressions("print if true then 1 else 2\n")
                .expect("properly parsed and typed"),
            type_boxes![1 => NonCoercedInteger]
        );
        assert_ne!(
        ...
        assert_eq!(
            Self::evaluated_expressions("print if false then 1 else 2\n")
                .expect("properly parsed and typed"),
            type_boxes![2 => NonCoercedInteger]
        );
        assert_ne!(
    ...
    fn test_unit_literal() {
        assert_eq!(
            Self::evaluated_expressions("print ()\n").expect("properly parsed and typed"),
            vec![TypeBox::Unit]
        );
        assert_eq!(
            Self::evaluated_expressions("print ((((()))))\n").expect("properly parsed and typed"),
    ...
    fn test_coerced_int_literal() {
        assert_eq!(
            Self::evaluated_expressions("print 0i8\n").expect("properly parsed and typed"),
            type_boxes![0 => Int8]
        );
        assert_eq!(
            Self::evaluated_expressions("print 0i16\n").expect("properly parsed and typed"),
            type_boxes![0 => Int16]
        );
        assert_eq!(
            Self::evaluated_expressions("print 0i32\n").expect("properly parsed and typed"),
            type_boxes![0 => Int32]
        );
        assert_eq!(
            Self::evaluated_expressions("print 0i64\n").expect("properly parsed and typed"),
            type_boxes![0 => Int64]
        );
    }
    ...
    fn test_infix_op_does_not_cause_panic_by_arithmetic_overflow() {
        ...
        assert_eq!(
            Self::evaluated_expressions("print 16i8 * 16i8\n").expect("properly parsed and typed"),
            type_boxes![0 => Int8]
        );
        assert_eq!(
            Self::evaluated_expressions("print 127i8 + 127i8 + 2i8\n")
                .expect("properly parsed and typed"),
            type_boxes![0 => Int8]
        );
        assert_eq!(
            Self::evaluated_expressions("print 0i8 - (127i8 + 127i8 + 1i8)\n")
                .expect("properly parsed and typed"),
            type_boxes![1 => Int8]
        );
        assert_eq!(
            Self::evaluated_expressions("print (127i8 + 127i8 + 2i8) / 1i8\n")
                .expect("properly parsed and typed"),
            type_boxes![0 => Int8]
        );
    }
    ...
    fn test_overflowed_literal() {
        ...
        macro_rules! gen {
            ($t:ty) => {{
                ...
                let e = Self::evaluated_expressions(src.as_str())
                    .expect_err("this operation should fail");
    ...
    fn test_variable_reassign() {
        assert_eq!(
            Self::evaluated_expressions("var a = 1\na = 2\nprint a\n")
                .expect("properly parsed and typed"),
            type_boxes![2 => NonCoercedInteger]
        );
    }
...
"#
    ...
    fn test_exit() {
        assert_eq!(
            Self::ast("exit\n").expect("properly parsed").statement,
            [Statement::Exit]
        );
        assert_eq!(
            Self::evaluated_expressions("exit\n").expect("properly parsed and typed"),
            []
        );
    }
    ...
    fn test_underscore_discard() {
        assert_eq!(
            Self::evaluated_expressions("var _ = 1\n").expect("properly parsed and typed"),
            []
        );
        assert_eq!(
            Self::evaluated_expressions("var a = block\n  print 1\n()\nend\n")
                .expect("FATAL: shouldn't fail"),
            type_boxes![ 1 => NonCoercedInteger ]
        );
        assert_eq!(
            Self::evaluated_expressions("var _ = block\n  print 1\n()\nend\n")
                .expect("properly parsed and typed"),
            type_boxes![ 1 => NonCoercedInteger ]
        );
        assert_eq!(
            Self::evaluated_expressions("var _ = _\n"),
            Err(TestFailureCause::Parser(ParserError::new(
                ParserErrorInner::UnderscoreCanNotBeRightHandExpression,
                SourcePosition::try_new((1, 9)).unwrap()
            )))
        );
    }
    ...
    fn test_type_alias() {
        ...
        assert_eq!(
            Self::evaluated_expressions("type Ik = Int32\n").expect("properly parsed and typed"),
            []
        );
        assert_eq!(
            Self::evaluated_expressions("type Ik = Int32\nvar t: Ik = 0i32\nprint t\n")
    ...
    fn test_shift() {
        ...
        assert_eq!(
            Self::evaluated_expressions("var t = 1i32 << 2i32\nprint t\n")
                .expect("properly parsed and typed"),
            type_boxes![4 => Int32]
        );
        assert_eq!(
        ...
        assert_eq!(
            Self::evaluated_expressions("var t = 4i32 >> 2i32\nprint t\n")
                .expect("properly parsed and typed"),
            type_boxes![1 => Int32]
        );
    }
    ...
    fn test_tuple_destruction() {
        // literal
        assert_eq!(
            Self::evaluated_expressions("var (a, b) = (1i32, 2i32)\nprint a\nprint b\n")
                .expect("properly parsed and typed"),
        ...
        assert_eq!(
            Self::evaluated_expressions("var (a, _) = (1i32, 2i32)\nprint a\n")
                .expect("properly parsed and typed"),
            type_boxes![1 => Int32]
        );
        assert_eq!(
            Self::evaluated_expressions("var (a, _) = (1i32, ())")
                .expect("properly parsed and typed"),
            type_boxes![]
        );
        assert_eq!(
            Self::evaluated_expressions("var (a, _) = (1i32, block\n()\nend)")
                .expect("properly parsed and typed"),
            type_boxes![]
        );
        assert_eq!(
        ...
        assert_eq!(
            Self::evaluated_expressions(
                "var (a, _) = (46i32, if true then 178i32 else 251i32)\nprint a\n"
            )
            .expect("properly parsed and typed"),
            type_boxes![46 => Int32]
        );
        ...
        // var
        assert_eq!(
            Self::evaluated_expressions("var z = (1i32, 2i32)\nvar (a, b) = z\nprint a\nprint b")
                .expect("properly parsed and typed"),
            type_boxes![1 => Int32, 2 => Int32]
        );
        assert_eq!(
            Self::evaluated_expressions(
                "var z = (1i32, 2i32, 3i32)\nvar (a, b, c) = z\nprint a\nprint b\nprint c"
            )
            .expect("properly parsed and typed"),
            type_boxes![1 => Int32, 2 => Int32, 3 => Int32]
        );
        ...
        assert_eq!(
            Self::evaluated_expressions(
                "var z = (1i32, (21i32, 30i32))\nvar (a, (b, c)) = z\nprint a\nprint b\nprint c"
            )
            .expect("properly parsed and typed"),
            type_boxes![1 => Int32, 21 => Int32, 30 => Int32],
            "var nest"
        );
