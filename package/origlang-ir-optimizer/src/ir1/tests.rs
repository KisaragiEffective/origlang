use crate::ir1::{
    EliminateAfterExit, FoldBinaryOperatorInvocationWithConstant, FoldIfWithConstantCondition,
    InlineSimpleBlock,
};
...
fn fold_binary_operator_is_recursive() {
    let opt_output = FoldBinaryOperatorInvocationWithConstant(vec![IR1::Output(
    ...
    assert_eq!(
        opt_output,
        vec![IR1::Output(TypedExpression::IntLiteral(
            TypedIntLiteral::Generic(6)
        ))]
    );
}
...
fn fold_comparison_between_integrals_into_boolean_literal() {
    ...
    assert_eq!(
        opt_output,
        [IR1::Output(TypedExpression::BooleanLiteral(true))]
    );
}

#[test]
fn fold_if_with_constant_condition() {
    let opt_output = FoldIfWithConstantCondition(vec![IR1::Output(TypedExpression::If {
...
fn inline_simple_block() {
    let opt_output = InlineSimpleBlock(vec![IR1::Output(TypedExpression::Block {
        inner: vec![],
    ...
    assert_eq!(opt_output, vec![IR1::Output(TypedExpression::UnitLiteral)]);
}
