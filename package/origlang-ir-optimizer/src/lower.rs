    ...
    fn compile_typed_expression(&self, te: TypedExpression) -> CompiledTypedExpression {
        match te {
            ...
            TypedExpression::Variable { ident, tp } => {
                CompiledTypedExpression::Variable { ident, tp }
            }
            TypedExpression::BinaryOperator {
                ...
                operator,
                return_type,
            } => CompiledTypedExpression::BinaryOperator {
            ...
            TypedExpression::Block {
                ...
                final_expression,
                return_type,
            } => CompiledTypedExpression::Block {
                inner: inner
                    .into_iter()
                    .flat_map(|x| self.compile_typed_statement(x))
                    .collect(),
            ...
            TypedExpression::Tuple { expressions } => CompiledTypedExpression::Tuple {
                expressions: expressions
                    .into_iter()
                    .map(|x| self.compile_typed_expression(x))
                    .collect(),
            },
            TypedExpression::ExtractTuple { expr, index } => {
                CompiledTypedExpression::ExtractTuple {
                    expr: Box::new(self.compile_typed_expression(*expr)),
                    index,
                }
            }
        }
    ...
    fn lower(&self, ir: Vec<IR1>) -> Vec<IR2> {
        ir.into_iter()
            .map(|x| match x {
                ...
                IR1::UpdateVariable { ident, value } => IR2::UpdateVariable {
                    ident,
                    value: self.compile_typed_expression(value),
                },
                IR1::PushScope => IR2::PushScope,
                IR1::PopScope => IR2::PopScope,
                IR1::Exit => IR2::Exit,
                IR1::EvalAndForget { expression } => IR2::EvalAndForget {
                    expression: self.compile_typed_expression(expression),
                },
            })
            .collect()
    }
