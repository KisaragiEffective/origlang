...
use origlang_source_span::{
    Pointed as WithPosition, Pointed, SourcePosition as SourcePos, SourcePosition,
};
...
impl Parser {
    ...
    pub fn parse(&self) -> Result<RootAst, ParserError> {
        ...
        while self
            .lexer
            .peek()
            .is_some_and(|x| x.data != Token::EndOfFile)
        {
...

        {
            let t = self
                .lexer
                .peek()
                .unwrap_or(&self.lexer.end_of_file_token())
                .clone();
            self.lexer.next();
            match t.data {
                ...
                other => Err(ParserError::new(
                    ParserErrorInner::UnconsumedToken { token: other },
                    t.position,
                )),
            }
        }
    ...
    fn parse_statement(&self) -> Result<Statement, ParserError> {
        ...
        let head1 = self.lexer.peek().ok_or_else(|| {
        ...
        let s = match head {
            Token::Identifier { .. } => self.parse_variable_assignment()?,
            Token::VarKeyword => self.parse_variable_declaration()?,
            Token::KeywordPrint => {
                // assuming expression
                self.lexer.next();
            ...
            Token::KeywordBlock => self.parse_block_scope()?,
            ...
            Token::KeywordType => {
                ...
                let Some(Token::Identifier { inner: aliased }) = aliased.map(|x| &x.data) else {
                    ...
                    return Err(Self::create_unexpected_token_error(
                ...
                let Ok(replace_with) = self.lexer.parse_fallible(|| self.parse_type()) else {
                    let p = self
                        ...
                        .clone();
                    return Err(Self::create_unexpected_token_error(
                        TokenKind::StartOfTypeSignature,
                        p.data,
                        p.position,
                    ));
                };
    ...
    fn parse_first(&self) -> Result<Expression, ParserError> {
        ...
        match &token.data {
            ...
            Token::SymLeftPar => {
                ...
                } else if let Ok(expr_tuple) =
                    self.lexer.parse_fallible(|| self.parse_tuple_expression())
                {
                    Ok(expr_tuple)
                } else {
            ...
            e => Err(Self::create_unexpected_token_error(
                TokenKind::First,
                e.clone(),
                token.position,
            )),
    ...
    fn parse_tuple_expression(&self) -> Result<Expression, ParserError> {
        self.lexer.parse_fallible(|| {
            ...
            while let Ok(e) = self.parse_lowest_precedence_expression() {
                buf.push(e);
                let peek = self.lexer.peek().ok_or_else(|| {
                    ParserError::new(ParserErrorInner::EndOfFileError, self.lexer.last_position)
                })?;
                if peek.data == Token::SymRightPar {
                    ...
                    return Err(Self::create_unexpected_token_error(
                        TokenKind::Only(Token::SymComma.display()),
                        peek.data.clone(),
                        peek.position,
                    ));
                }
            ...
            if bl == 0 {
                ...
                return Err(ParserError::new(
                    ParserErrorInner::InsufficientElementsForTupleLiteral(
                        UnexpectedTupleLiteralElementCount::Zero,
                    ),
                    self.lexer
                        .peek()
                        .ok_or_else(|| {
                            ParserError::new(
                                ParserErrorInner::EndOfFileError,
                ...
                return Err(ParserError::new(
                    ParserErrorInner::InsufficientElementsForTupleLiteral(
                        UnexpectedTupleLiteralElementCount::One,
                    ),
                    self.lexer
                        .peek()
                        .ok_or_else(|| {
            ...
            Ok(Expression::Tuple { expressions: buf })
        })
    }
    ...
    fn parse_multiplicative(&self) -> Result<Expression, ParserError> {
        ...
        let asterisk_or_slash =
            |token: &Token| token == &Token::SymAsterisk || token == &Token::SymSlash;
        ...
        if asterisk_or_slash(&next_token.data) {
            ...
            let get_operator_from_token = |token: &WithPosition<Token>| match &token.data {
                Token::SymAsterisk => Ok(BinaryOperatorKind::Multiply),
                Token::SymSlash => Ok(BinaryOperatorKind::Divide),
                e => Err(Self::create_unexpected_token_error(
                    TokenKind::MultiplicativeOps,
                    e.clone(),
                    token.position,
                )),
            };
            ...
            let mut operator_token = self.lexer.peek().ok_or_else(|| {
                ParserError::new(ParserErrorInner::EndOfFileError, self.lexer.last_position)
            })?;
            while asterisk_or_slash(&operator_token.data) {
                ...
                operator_token = self.lexer.peek().ok_or_else(|| {
                    ParserError::new(ParserErrorInner::EndOfFileError, self.lexer.last_position)
                })?;
            }
            Ok(acc)
    ...
    fn parse_additive(&self) -> Result<Expression, ParserError> {
        ...
        let plus_or_minus = |token: &Token| token == &Token::SymPlus || token == &Token::SymMinus;
    ...
    fn parse_shift_expression(&self) -> Result<Expression, ParserError> {
        ...
        let is_relation_operator =
            |token: &Token| matches!(token, Token::PartLessLess | Token::PartMoreMore);
    ...
    fn parse_equality_expression(&self) -> Result<Expression, ParserError> {
        ...
        let is_relation_operator =
            |token: &Token| matches!(token, Token::PartEqEq | Token::PartBangEq);
    ...
    fn parse_int_literal(&self) -> Result<(i64, Option<&str>), ParserError> {
        ...
        self.lexer.next();
        let Some(Pointed {
            data: Token::Digits { sequence, suffix },
            position,
        }) = n
        else {
        ...
        let x = sequence.as_str().parse::<i64>().map_err(|e| {
            ParserError::new(
                ParserErrorInner::UnParsableIntLiteral { error: e },
                n.map_or(self.lexer.last_position, |x| x.position),
            )
        })?;

        fn check_bounds<As: Bounded + Into<i64>>(
            ty: &str,
            token_pos: SourcePos,
            v: i64,
            ...
            Token::SymLeftPar => {
                ...
                self.lexer.parse_fallible(|| {
                    ...
                    if vec.len() < 2 {
                        ...
                        Err(ParserError::new(
                            ParserErrorInner::InsufficientElementsForTupleLiteral(match l {
    ...
    fn read_and_consume_or_report_unexpected_token(
        &self,
        token: &Token,
    ) -> Result<(), ParserError> {
        let peek = self.lexer.peek().ok_or_else(|| {
        ...
        if &peek.data == token {
            ...
            Err(Self::create_unexpected_token_error(
                TokenKind::only(token),
                peek.data.clone(),
                peek.position,
            ))
        }
    ...
    const fn create_unexpected_token_error(
