...
現状コメントは存在しません。将来的には、コメントが組み込みの文法として実装される見込みです。

それまでの間、ダーティハックとしてダミーの変数に文字列リテラルを代入して意図を示すことができます：

```
var discard = "------ vvvvvv subroutine vvvvvv ------"
```
...
変数というのは箱です。`var ident = expr`という構文で宣言することができます。型注釈は置くことができません。

変数の宣言時に型が固定されます。異なる型として推論される値を代入するとコンパイルエラーになります。
